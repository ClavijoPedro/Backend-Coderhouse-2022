...
class ProductsDaoMongoDB extends DAO{
    ...
    //singleton
    static getInstance(model, schema){
        if(!instance){
           instance = new ProductsDaoMongoDB(model, schema)
        }
        return instance
    }
    ...
    async save(itm){
        try{
           ...
           await item.save()
           logger.info(`item agregado id: ${item.id}`) 
           return item
        }
        catch(error){ logger.error(error) }
    }
    ...
    async updateById(id, itmUpdate){
        try{
            const updatedItem = await this.model.findByIdAndUpdate(id, itmUpdate)
            return updatedItem
        }
        catch(error){ logger.error(error) }
    }
    ...
    async deleteById(id){
        try{
            const detedPoroduct = await this.model.findByIdAndDelete(id)
            return detedPoroduct
        }
        catch(error){ logger.error(error) }
    }
